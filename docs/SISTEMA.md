...
> Gerada em 2026-08-11 07:04 -03 por `scripts/gerar_documentacao.mjs`. Reflete o estado real do código e do banco. **Não editar à mão.**
...
| `arquivos_proc_feitos` | 248.572 | 3 (numero_controle, n, feito_em) |
| `arquivos_sc` | 652.209 | 14 (cnpj, ano, seq, sequencial_documento, cod_ibge, tipo_documento_id, tipo_documento, titulo…) |
...
| `coleta_incremental_log` | 21 | 9 (rodada_em, uf, janela_ini, janela_fim, vistos, mudaram, iguais, novos…) |
...
| `contratacoes_sc` | 248.964 | 35 (cod_ibge, cnpj, ano, seq, esfera, plataforma, modalidade_id, modalidade…) |
| `contrato_sc` | 10.559 | 30 (cnpj, ano, seq, cod_ibge, uf, municipio_nome, numero_controle_pncp, numero_controle_compra…) |
| `contratos_sc` | 2.220.065 | 14 (id, cod_ibge, numero_controle_compra, cnpj_compra, ano_compra, seq_compra, fornecedor, ni_fornecedor…) |
...
| `frescor_log` | 35 | 6 (id, rodado_em, total, ok, resumo, problemas) |
...
| `item_resultado_sc` | 1.692.665 | 23 (cod_ibge, cnpj, ano, seq, numero, sequencial_resultado, ni_fornecedor, nome_razao_social_fornecedor…) |
| `itens_classificacao_sc` | 733.326 | 14 (descr_norm, tipo, cat_nivel, cat_cod, cat_nome, cat_classe, shared, cobertura…) |
| `itens_proc_feitos` | 248.932 | 4 (numero_controle, n, feito_em, versao) |
| `itens_sc` | 2.271.219 | 49 (cod_ibge, cnpj, ano, seq, numero, descricao, unidade, quantidade…) |
...
| `marca_ancora_feitas` | 21.482 | 6 (cnpj, ano, seq, n_marca, via_haiku, feito_em) |
...
| `mislabel_unidade_sc` | 1.919 | 13 (cod_ibge, nome_pdm, descricao, unidade, unit_homologado, unidade_basica, pub, med…) |
...
| `notificacao_log` | 5.673 | 11 (id, cod_ibge, alerta_id, chave_delta, destinatario_id, canal, severidade, enviado_em…) |
...
| `pg_stat_statements` | 4.886 | 52 (userid, dbid, toplevel, queryid, query, plans, total_plan_time, min_plan_time…) |
...
| `pncp_evento` | 106.694 | 22 (cnpj, ano, seq, cod_ibge, categoria, acao, item_numero, resultado_sequencial…) |
...
| `processos_sc` | 248.964 | 11 (cnpj_orgao, ano, sequencial, cod_ibge, numero_controle, modalidade_id, modalidade, objeto…) |
...
|---|---|---|---|---|
| Outorgas de uso da água por município (ANA, finalidade+série) | ana | — | há 2d | em dia |
...
| Geração distribuída de energia por município (ANEEL, série) | aneel | — | há 5h | em dia |
| CFEM — royalties de mineração por município (ANM, distribuição) | anm | 2026 | há 2d | em dia |
| Preços de combustíveis por município (ANP, semestral, série 2004+) | anp | 2021 | há 2d | em dia |
| Vendas de combustíveis por município (ANP, série 1990+) | anp | 2024 | há 5h | em dia |
| Cobertura de planos de saúde por município (ANS — pressão sobre o SUS) | ans | 2026 | há 2d | em dia |
| CMED/Anvisa PMVG — preço-teto legal de medicamentos (Conformidade Gov, auto-descobre URL); referência nacional p/ sobrepreço em saúde (SC = ICMS 17%) | anvisa | 2025 | há 7d | em dia |
| Desastres S2ID por município (Atlas Digital CEPED-UFSC/MIDR, série 1991+) | atlas | — | há 5h | em dia |
| IDHM + subíndices por município (Atlas Brasil PNUD, Censo 2010) | atlasbrasil | 2025 | há 2d | em dia |
| Sistema financeiro por município (BCB Olinda — agências/cooperativas/correspondentes + Pix série) | bcb | — | há 7d | em dia |
| ESTBAN — volumes bancários por município (BCB, crédito/poupança, série) | bcb | — | há 7d | em dia |
...
| BNDES desembolsos por município (crédito produtivo, série 1995+) | bndes | 2026 | há 2d | em dia |
| Espelho completo CADPREV (37 recursos: DAIR/DIPR/DRAA/RPPS_*) | cadprev | — | há 14d | em dia |
...
| Habitação — MCMV (Minha Casa Minha Vida), unidades financiadas por município (Min. Cidades, gov.br/cidades — sem WAF); alimenta o casamento oportunidade×necessidade | cidades | 2025 | há 2d | em dia |
| Saneamento SINISA por município (água/esgoto/resíduos, Min. Cidades, ref 2024) | cidades | 2024 | há 2d | em dia |
| CNES — rede de saúde (Min. Saúde) | cnes | 2026 | há 5h | em dia |
| Unidades de conservação por município (MMA CNUC, interseção PostGIS) | cnuc | — | há 2d | em dia |
...
| Bancada federal do estado (deputados + senadores) p/ módulo de Captação de Emendas — APIs Câmara + Senado | congresso | 2025 | há 7d | em dia |
...
| Equipamentos médicos por estabelecimento (CNES EQ) | datasus | 2024 | há 2d | em dia |
| Equipes de saúde (ESF) por município/estabelecimento (CNES EP) | datasus | 2024 | há 2d | em dia |
| Estabelecimentos de saúde por município (CNES — rede p/ regulação, API DEMAS) | datasus | 2025 | há 5h | em dia |
| Leitos hospitalares por estabelecimento (CNES LT) | datasus | 2024 | há 2d | em dia |
| Profissionais de saúde por município/estabelecimento (CNES PF) | datasus | 2024 | há 36d | em dia |
| Medicamentos de alto custo CEAF por município (SIA grupo 06) | datasus | — | há 5h | em dia |
| Previne Brasil — indicadores APS (SISAB) | datasus | — | há 7d | em dia |
...
| Produção ambulatorial SUS por complexidade (DATASUS SIA + SIGTAP) | datasus | — | há 2d | em dia |
| Internações SUS por município (DATASUS SIH DBC mensal) | datasus | 2024 | há 2d | em dia |
| SIH — produção hospitalar (DATASUS TabNet) | datasus | 2025 | há 5h | em dia |
| Óbitos por município (DATASUS SIM, descompressor DBC próprio) | datasus | 2024 | há 2d | em dia |
| Agravos de notificação SINAN por município (tuberculose/hanseníase/violência) | datasus | 2024 | há 2d | em dia |
| Nascimentos por município (DATASUS SINASC DBC) | datasus | 2022 | há 2d | em dia |
...
| Análise de compras por PREÇO UNITÁRIO — livro de preços de referência de SC + sobrepreço por município (derivado de itens_sc, via SQL) | derivado | 2025 | há 7d | em dia |
| Red flags de fornecedores — concentração + sanção (CEIS/CNEP) + sobrepreço por fornecedor (derivado de contratos/itens/sancoes, via SQL) | derivado | 2025 | há 7d | em dia |
| Re-derivação por fatia (evento → derivadas do ente) | derivado | 2025 | há 4h | em dia |
| Indícios de sobrepreço em medicamentos (compras do município vs teto legal PMVG, por substância+dosagem); derivado de cmed_pmvg + itens_sc | derivado | 2025 | há 2d | em dia |
...
| Variação INTERNA de preços — mesmo município comprou o mesmo item a preços diferentes (derivado de itens_sc, via SQL) | derivado | 2025 | há 7d | em dia |
| Bolsa Atleta por município (Min. Esporte — atletas + valor) | esporte | 2025 | há 1h | pendente |
| Matrículas FUNDEB oficiais (FNDE Antonieta de Barros) | fnde | 2026 | há 2d | em dia |
| Parâmetros FUNDEB — fatores/VAAT/VAAR (FNDE) | fnde | 2026 | há 2d | em dia |
| PDDE por município (FNDE — Plataforma Antonieta de Barros) | fnde | 2024 | há 2d | em dia |
...
| PNLD reserva técnica — demanda de livros (FNDE — Antonieta de Barros) | fnde | 2025 | há 7d | em dia |
| Repasses federais FNS por bloco (Consulta Consolidada) | fns | 2026 | há 2d | em dia |
| Autos de infração ambiental por município (IBAMA, série 1990+) | ibama | — | há 7d | em dia |
...
| População por cor/raça Censo 2022 (IBGE SIDRA 9605) | ibge | 2025 | há 2d | em dia |
...
| IBGE MUNIC — instrumentos de gestão (planos/conselhos/fundos/instrumentos legais) por município, da BASE DE DADOS oficial (xlsx), não SIDRA | ibge | 2025 | há 1d | em dia |
...
| Saneamento por município (água/esgoto/lixo) — IBGE Censo 2022 via SIDRA (cobertura por domicílio) | ibge | 2025 | há 2d | em dia |
| Setores censitários Censo 2022 — perfil intraurbano (IBGE FTP) | ibge | 2025 | há 5h | em dia |
| % crianças 0-14 por setor → variável do mapa intraurbano (IBGE demografia) | ibge | 2025 | há 2d | em dia |
| Malha (polígonos) dos setores → mapa choropleth (IBGE GPKG) | ibge | 2025 | há 1d | em dia |
| % idosos por setor censitário → variável do mapa intraurbano (IBGE demografia) | ibge | 2025 | há 2d | em dia |
| Museus por município (IBRAM MuseusBr) | ibram | 2025 | há 2d | em dia |
| Assentamentos reforma agrária por município (INCRA/MDA SIPRA) | incra | — | há 5h | em dia |
| Censo Escolar — matrículas (INEP Sinopse) | inep | 2025 | há 1d | em dia |
...
| Escolas por município + infraestrutura (INEP Censo Escolar) | inep | 2025 | há 1d | em dia |
| Taxa de evasão por etapa (INEP Fluxo/Transição) | inep | 2025 | há 34d | em dia |
| FNDE liberações por município (SIMAD, Playwright) | inep | 2025 | há 5h | em dia |
| IDEB — indicadores educacionais (INEP) | inep | 2025 | há 2d | em dia |
| Indicadores educacionais INEP (AFD/TDI/ATU/rendimento por município) | inep | 2025 | há 1d | em dia |
| Indicadores INEP por escola (georreferenciado) | inep | 2025 | há 1d | em dia |
...
| Desmatamento PRODES por município (INPE, interseção PostGIS) | inpe | 2025 | há 2d | em dia |
...
| CAF — Cadastro Nacional da Agricultura Familiar (ex-DAP) por município: nº de agricultores familiares (MDA, XLSX mensal) | mda | 2025 | há 7d | em dia |
| IGD-M gestão PBF/CadÚnico por município (MDS/SAGI Solr) | mds | — | há 5h | em dia |
| SUAS repasse+SALDO na mesa por município (MDS/SAGI Solr) — recurso não usado | mds | 2025 | há 7d | em dia |
...
| RAIS — estoque de emprego formal por município (MTE, anual) | mte | 2025 | há 1d | em dia |
...
| Equipamentos esportivos georreferenciados (OSM — mapa camada Esporte) | osm | — | há 2d | em dia |
| Equipamentos segurança/justiça/defesa civil georreferenciados (OSM + SAP/SC): polícia, bombeiros, defesa civil, prisional, socioeducativo | osm | — | há 2d | em dia |
...
| Busca diária do PNCP (compras/contratos/atas — ano corrente) | pncp | — | há 33d | em dia |
| Contratos (PNCP ano corrente, append) | pncp | 2027 | há 25d | pendente |
...
| Preço de referência por item (mediana SC) + classificação ata/efetivada — base da análise de preços | pncp | 2025 | há 49d | em dia |
...
| Sazonalidade de preço por categoria (melhor mês de compra, SC) | pncp | 2025 | há 48d | em dia |
| Acidentes em rodovias federais por município (PRF DATATRAN, série 2015+) | prf | 2025 | há 7d | em dia |
| Localidade dos fornecedores (CNPJ→UF/município) | receita | — | há 7d | em dia |
| Arrecadação federal por município (RFB, série 2019+) | rfb | 2025 | há 2d | em dia |
| Frota de veículos por município (SENATRAN) | senatran | 2025 | há 2d | em dia |
| Qualidade (15 indicadores) + Vínculo/CVAT — novo cofinanciamento (SIAPS) | siaps | 2025 | há 5h | em dia |
| CAR — Cadastro Ambiental Rural: nº de imóveis rurais por município (SICAR GeoServer WFS, contagem por município) | sicar | 2025 | há 2d | em dia |
| Acompanhamento intra-anual da execução (RREO do bimestre vigente) — receita prevista×realizada e despesa orçada×empenhada por município | siconfi | 2025 | há 3d | em dia |
| Acompanhamento por função (intra-anual) — orçado×realizado por função até o bimestre vigente (RREO Anexo 02 parcial) | siconfi | 2025 | há 3d | em dia |
| CNPJs do governo municipal (SICONFI+órgãos+RPPS) | siconfi | 2025 | há 3d | em dia |
...
| MSC ancorada ao RREO — despesa empenhada por natureza e fonte (forma da MSC × total exato do RREO; reconcilia por construção) | siconfi | 2025 | há 3d | em dia |
...
| Estatísticas vitais por município (IBGE Registro Civil — nascidos/óbitos, série) | sidra | 2024 | há 6d | em dia |
| Produção agropecuária (PAM/PPM) + empresas (CEMPRE) por município (IBGE SIDRA) | sidra | — | há 2d | em dia |
| Vítimas de crimes violentos letais por município (SINESP/MJSP, dados abertos) | sinesp | — | há 37d | em dia |
...
| Indicadores Previne + ISF (SISAB indicadorPainel — 10 quadrimestres) | sisab | 2025 | há 2d | em dia |
| Produção da APS (SISAB — fichas aprovadas, série mensal 2021+) | sisab | 2025 | há 7d | em dia |
| SNIS Água/Esgoto por município e prestador (atendimento, perdas, tratamento) — app Ministério das Cidades via Playwright; state-agnostic (UF/ANO) | snis | 2025 | há 2d | em dia |
...
| Ranking da Qualidade da Informação Fiscal (Tesouro) por município | tesouro | 2025 | há 2d | em dia |
...
| Convênios e contratos de repasse por município (SICONV/Transferegov, repositório detru — proposta+convenio) | transferegov | 2025 | há 13d | em dia |
...
- status: **ok** · registros suspeitos (excluídos): 6 · sobrepreço unitário: 18882
