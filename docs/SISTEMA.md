...
> Gerada em 2026-08-10 06:34 -03 por `scripts/gerar_documentacao.mjs`. Reflete o estado real do código e do banco. **Não editar à mão.**
...
| `arquivo_texto_sc` | 649.752 | 17 (cnpj, ano, seq, sequencial_documento, cod_ibge, tipo_documento, titulo, texto…) |
| `arquivos_proc_feitos` | 248.571 | 3 (numero_controle, n, feito_em) |
| `arquivos_sc` | 652.208 | 14 (cnpj, ano, seq, sequencial_documento, cod_ibge, tipo_documento_id, tipo_documento, titulo…) |
...
| `cnes_profissionais_estab` | 7.807 | 3 (codigo_cnes, profissionais, atualizado) |
...
| `coleta_incremental_log` | 20 | 9 (rodada_em, uf, janela_ini, janela_fim, vistos, mudaram, iguais, novos…) |
...
| `contratacoes_sc` | 248.572 | 35 (cod_ibge, cnpj, ano, seq, esfera, plataforma, modalidade_id, modalidade…) |
| `contrato_sc` | 10.559 | 30 (cnpj, ano, seq, cod_ibge, uf, municipio_nome, numero_controle_pncp, numero_controle_compra…) |
| `contratos_sc` | 2.114.250 | 14 (id, cod_ibge, numero_controle_compra, cnpj_compra, ano_compra, seq_compra, fornecedor, ni_fornecedor…) |
| `contratos_sc_feitos` | 295 | 2 (cod_ibge, n) |
| `contratos_sc_feitos_inc` | 295 | 2 (cod_ibge, n) |
| `convenios_captados_sc` | 43.238 | 12 (cod_ibge, id, numero, objeto, orgao, situacao, valor, valor_liberado…) |
...
| `frescor_log` | 28 | 6 (id, rodado_em, total, ok, resumo, problemas) |
...
| `item_apresentacao_desc_sc` | 47.878 | 7 (chave, unidade_basica, fator, conf, metodo, n_itens, atualizado) |
| `item_apresentacao_sc` | 3.052 | 9 (unidade, unidade_basica, fator, forma, dimensao, conf, metodo, n_itens…) |
...
| `item_resultado_sc` | 1.690.238 | 23 (cod_ibge, cnpj, ano, seq, numero, sequencial_resultado, ni_fornecedor, nome_razao_social_fornecedor…) |
| `itens_classificacao_sc` | 733.313 | 14 (descr_norm, tipo, cat_nivel, cat_cod, cat_nome, cat_classe, shared, cobertura…) |
| `itens_proc_feitos` | 248.539 | 4 (numero_controle, n, feito_em, versao) |
| `itens_sc` | 2.267.832 | 49 (cod_ibge, cnpj, ano, seq, numero, descricao, unidade, quantidade…) |
...
| `marca_ancora_feitas` | 17.389 | 6 (cnpj, ano, seq, n_marca, via_haiku, feito_em) |
...
| `medicamentos_alto_custo_sc` | 0 | 6 (cod_ibge, periodo, valor, quantidade, top_meds, atualizado) |
...
| `mislabel_unidade_sc` | 1.900 | 13 (cod_ibge, nome_pdm, descricao, unidade, unit_homologado, unidade_basica, pub, med…) |
...
| `notificacao_log` | 5.476 | 11 (id, cod_ibge, alerta_id, chave_delta, destinatario_id, canal, severidade, enviado_em…) |
...
| `pg_stat_statements` | 1.926 | 52 (userid, dbid, toplevel, queryid, query, plans, total_plan_time, min_plan_time…) |
...
| `pncp_evento` | 98.604 | 22 (cnpj, ano, seq, cod_ibge, categoria, acao, item_numero, resultado_sequencial…) |
...
| `precos_referencia_basica_sc` | 5.326 | 12 (codigo_pdm, nome_pdm, nome_classe, unidade_basica, forma, n_compras, n_munis, n_excluidos…) |
...
| `processos_sc` | 248.572 | 11 (cnpj_orgao, ano, sequencial, cod_ibge, numero_controle, modalidade_id, modalidade, objeto…) |
...
| `saeb_sc` | 0 | 7 (cod_ibge, ano, etapa, rede, matematica, portugues, atualizado) |
...
| `sia_producao_sc` | 0 | 10 (cod_ibge, periodo, q_basica, v_basica, q_media, v_media, q_alta, v_alta…) |
...
| `sinan_agravos_sc` | 224 | 5 (cod_ibge, agravo, ano, casos, atualizado) |
...
|---|---|---|---|---|
| Outorgas de uso da água por município (ANA, finalidade+série) | ana | — | há 1d | em dia |
...
| Geração distribuída de energia por município (ANEEL, série) | aneel | — | há 2h | em dia |
| CFEM — royalties de mineração por município (ANM, distribuição) | anm | 2026 | há 1d | em dia |
| Preços de combustíveis por município (ANP, semestral, série 2004+) | anp | 2021 | há 1d | em dia |
| Vendas de combustíveis por município (ANP, série 1990+) | anp | 2024 | há 5h | em dia |
| Cobertura de planos de saúde por município (ANS — pressão sobre o SUS) | ans | 2026 | há 1d | em dia |
| CMED/Anvisa PMVG — preço-teto legal de medicamentos (Conformidade Gov, auto-descobre URL); referência nacional p/ sobrepreço em saúde (SC = ICMS 17%) | anvisa | 2025 | há 6d | em dia |
| Desastres S2ID por município (Atlas Digital CEPED-UFSC/MIDR, série 1991+) | atlas | — | há 5h | em dia |
| IDHM + subíndices por município (Atlas Brasil PNUD, Censo 2010) | atlasbrasil | 2025 | há 1d | em dia |
| Sistema financeiro por município (BCB Olinda — agências/cooperativas/correspondentes + Pix série) | bcb | — | há 6d | em dia |
| ESTBAN — volumes bancários por município (BCB, crédito/poupança, série) | bcb | — | há 6d | em dia |
...
| BNDES desembolsos por município (crédito produtivo, série 1995+) | bndes | 2026 | há 1d | em dia |
| Espelho completo CADPREV (37 recursos: DAIR/DIPR/DRAA/RPPS_*) | cadprev | — | há 13d | em dia |
...
| Habitação — MCMV (Minha Casa Minha Vida), unidades financiadas por município (Min. Cidades, gov.br/cidades — sem WAF); alimenta o casamento oportunidade×necessidade | cidades | 2025 | há 1d | em dia |
| Saneamento SINISA por município (água/esgoto/resíduos, Min. Cidades, ref 2024) | cidades | 2024 | há 1d | em dia |
| CNES — rede de saúde (Min. Saúde) | cnes | 2026 | há 5h | em dia |
| Unidades de conservação por município (MMA CNUC, interseção PostGIS) | cnuc | — | há 1d | em dia |
| Catálogo oficial CATMAT/CATSER (Compras.gov.br) — espinha de classificação | compras | — | há 5h | em dia |
| Referência NACIONAL de preços por CATMAT (Painel de Preços/Compras.gov.br), por unidade e por forma (avulso×escala) — enriquece o Banco de Preços e o sobrepreço | compras | 2025 | há 0h | pendente |
| PAA compras agricultura familiar por município (Conab) | conab | 2025 | há 5h | em dia |
| Bancada federal do estado (deputados + senadores) p/ módulo de Captação de Emendas — APIs Câmara + Senado | congresso | 2025 | há 6d | em dia |
...
| APAC oncologia e diálise por município (SIA-APAC) | datasus | — | há 3h | em dia |
| Equipamentos médicos por estabelecimento (CNES EQ) | datasus | 2024 | há 1d | em dia |
| Equipes de saúde (ESF) por município/estabelecimento (CNES EP) | datasus | 2024 | há 1d | em dia |
| Estabelecimentos de saúde por município (CNES — rede p/ regulação, API DEMAS) | datasus | 2025 | há 5h | em dia |
| Leitos hospitalares por estabelecimento (CNES LT) | datasus | 2024 | há 1d | em dia |
| Profissionais de saúde por município/estabelecimento (CNES PF) | datasus | 2024 | há 35d | em dia |
| Medicamentos de alto custo CEAF por município (SIA grupo 06) | datasus | — | há 5h | em dia |
| Previne Brasil — indicadores APS (SISAB) | datasus | — | há 6d | em dia |
| Saúde mental CAPS/RAAS por município (SIA-RAAS-PS) | datasus | — | há 3h | em dia |
| SIA — produção ambulatorial (DATASUS) | datasus | 2025 | há 3h | em dia |
| Produção ambulatorial SUS por complexidade (DATASUS SIA + SIGTAP) | datasus | — | há 1d | em dia |
| Internações SUS por município (DATASUS SIH DBC mensal) | datasus | 2024 | há 1d | em dia |
| SIH — produção hospitalar (DATASUS TabNet) | datasus | 2025 | há 5h | em dia |
| Óbitos por município (DATASUS SIM, descompressor DBC próprio) | datasus | 2024 | há 1d | em dia |
| Agravos de notificação SINAN por município (tuberculose/hanseníase/violência) | datasus | 2023 | há 23h | em dia |
| Nascimentos por município (DATASUS SINASC DBC) | datasus | 2022 | há 23h | em dia |
...
| Referência por UNIDADE BÁSICA (Passe 2) — CATMAT+base+forma, mediana + curadoria IQR (IN 65 art.6); derivado de itens_sc + item_catmat_map + apresentação | derivado | 2025 | há 2h | em dia |
| Análise de compras por PREÇO UNITÁRIO — livro de preços de referência de SC + sobrepreço por município (derivado de itens_sc, via SQL) | derivado | 2025 | há 6d | em dia |
| Red flags de fornecedores — concentração + sanção (CEIS/CNEP) + sobrepreço por fornecedor (derivado de contratos/itens/sancoes, via SQL) | derivado | 2025 | há 6d | em dia |
| Re-derivação por fatia (evento → derivadas do ente) | derivado | 2025 | há 3h | em dia |
| Indícios de sobrepreço em medicamentos (compras do município vs teto legal PMVG, por substância+dosagem); derivado de cmed_pmvg + itens_sc | derivado | 2025 | há 1d | em dia |
| Reconstrução do sobrepreço com DUPLO benchmark — mediana de SC + referência nacional + desvio/CV (IN 65); derivado de itens_sc + precos_referencia_sc + precos_nacional_ref | derivado | 2025 | há 5d | em dia |
| Variação INTERNA de preços — mesmo município comprou o mesmo item a preços diferentes (derivado de itens_sc, via SQL) | derivado | 2025 | há 6d | em dia |
| Bolsa Atleta por município (Min. Esporte — atletas + valor) | esporte | 2025 | há 0h | pendente |
| Matrículas FUNDEB oficiais (FNDE Antonieta de Barros) | fnde | 2026 | há 1d | em dia |
| Parâmetros FUNDEB — fatores/VAAT/VAAR (FNDE) | fnde | 2026 | há 1d | em dia |
...
| PNLD reserva técnica — demanda de livros (FNDE — Antonieta de Barros) | fnde | 2025 | há 6d | em dia |
| Repasses federais FNS por bloco (Consulta Consolidada) | fns | 2026 | há 1d | em dia |
| Autos de infração ambiental por município (IBAMA, série 1990+) | ibama | — | há 6d | em dia |
...
| População por cor/raça Censo 2022 (IBGE SIDRA 9605) | ibge | 2025 | há 1d | em dia |
...
| IBGE MUNIC — instrumentos de gestão (planos/conselhos/fundos/instrumentos legais) por município, da BASE DE DADOS oficial (xlsx), não SIDRA | ibge | 2025 | há 2h | em dia |
...
| Saneamento por município (água/esgoto/lixo) — IBGE Censo 2022 via SIDRA (cobertura por domicílio) | ibge | 2025 | há 1d | em dia |
| Setores censitários Censo 2022 — perfil intraurbano (IBGE FTP) | ibge | 2025 | há 5h | em dia |
| % crianças 0-14 por setor → variável do mapa intraurbano (IBGE demografia) | ibge | 2025 | há 1d | em dia |
| Malha (polígonos) dos setores → mapa choropleth (IBGE GPKG) | ibge | 2025 | há 4h | em dia |
| % idosos por setor censitário → variável do mapa intraurbano (IBGE demografia) | ibge | 2025 | há 1d | em dia |
| Museus por município (IBRAM MuseusBr) | ibram | 2025 | há 1d | em dia |
| Assentamentos reforma agrária por município (INCRA/MDA SIPRA) | incra | — | há 5h | em dia |
| Censo Escolar — matrículas (INEP Sinopse) | inep | 2025 | há 1h | em dia |
| Educação especial/AEE por município (INEP Censo microdata) | inep | 2025 | há 2h | em dia |
| Número de turmas por etapa/rede (INEP Censo Escolar) | inep | 2025 | há 2h | em dia |
| Escolas por município + infraestrutura (INEP Censo Escolar) | inep | 2025 | há 0h | em dia |
| Taxa de evasão por etapa (INEP Fluxo/Transição) | inep | 2025 | há 33d | em dia |
| FNDE liberações por município (SIMAD, Playwright) | inep | 2025 | há 5h | em dia |
| IDEB — indicadores educacionais (INEP) | inep | 2025 | há 1d | em dia |
| Indicadores educacionais INEP (AFD/TDI/ATU/rendimento por município) | inep | 2025 | há 2h | em dia |
| Indicadores INEP por escola (georreferenciado) | inep | 2025 | há 1h | em dia |
...
| Desmatamento PRODES por município (INPE, interseção PostGIS) | inpe | 2025 | há 1d | em dia |
| Focos de calor por município (INPE BDQueimadas, mensal) | inpe | 2026 | há 3d | em dia |
| IEGM — qualidade da gestão (TCE-SC/IRB, dados abertos) | irb | 2025 | há 1d | em dia |
...
| CAF — Cadastro Nacional da Agricultura Familiar (ex-DAP) por município: nº de agricultores familiares (MDA, XLSX mensal) | mda | 2025 | há 6d | em dia |
| IGD-M gestão PBF/CadÚnico por município (MDS/SAGI Solr) | mds | — | há 5h | em dia |
| SUAS repasse+SALDO na mesa por município (MDS/SAGI Solr) — recurso não usado | mds | 2025 | há 6d | em dia |
...
| RAIS — estoque de emprego formal por município (MTE, anual) | mte | 2025 | há 2h | em dia |
...
| Equipamentos esportivos georreferenciados (OSM — mapa camada Esporte) | osm | — | há 1d | em dia |
| Equipamentos segurança/justiça/defesa civil georreferenciados (OSM + SAP/SC): polícia, bombeiros, defesa civil, prisional, socioeducativo | osm | — | há 1d | em dia |
...
| Busca diária do PNCP (compras/contratos/atas — ano corrente) | pncp | — | há 32d | em dia |
| Contratos (PNCP ano corrente, append) | pncp | 2027 | há 24d | em dia |
...
| PCA (PNCP) | pncp | — | há 5h | em dia |
| Preço de referência por item (mediana SC) + classificação ata/efetivada — base da análise de preços | pncp | 2025 | há 48d | em dia |
...
| Sazonalidade de preço por categoria (melhor mês de compra, SC) | pncp | 2025 | há 47d | em dia |
| Acidentes em rodovias federais por município (PRF DATATRAN, série 2015+) | prf | 2025 | há 6d | em dia |
| Localidade dos fornecedores (CNPJ→UF/município) | receita | — | há 6d | em dia |
| Arrecadação federal por município (RFB, série 2019+) | rfb | 2025 | há 1d | em dia |
| Frota de veículos por município (SENATRAN) | senatran | 2025 | há 1d | em dia |
| Qualidade (15 indicadores) + Vínculo/CVAT — novo cofinanciamento (SIAPS) | siaps | 2025 | há 5h | em dia |
| CAR — Cadastro Ambiental Rural: nº de imóveis rurais por município (SICAR GeoServer WFS, contagem por município) | sicar | 2025 | há 1d | em dia |
| Acompanhamento intra-anual da execução (RREO do bimestre vigente) — receita prevista×realizada e despesa orçada×empenhada por município | siconfi | 2025 | há 2d | em dia |
| Acompanhamento por função (intra-anual) — orçado×realizado por função até o bimestre vigente (RREO Anexo 02 parcial) | siconfi | 2025 | há 2d | em dia |
| CNPJs do governo municipal (SICONFI+órgãos+RPPS) | siconfi | 2025 | há 2d | em dia |
...
| MSC ancorada ao RREO — despesa empenhada por natureza e fonte (forma da MSC × total exato do RREO; reconcilia por construção) | siconfi | 2025 | há 2d | em dia |
...
| Salário-Educação por município (SICONFI DCA) | siconfi | 2025 | há 1d | em dia |
| Estatísticas vitais por município (IBGE Registro Civil — nascidos/óbitos, série) | sidra | 2024 | há 5d | em dia |
| Produção agropecuária (PAM/PPM) + empresas (CEMPRE) por município (IBGE SIDRA) | sidra | — | há 1d | em dia |
| Vítimas de crimes violentos letais por município (SINESP/MJSP, dados abertos) | sinesp | — | há 36d | em dia |
| Catálogo de Ações Orçamentárias Federais (SIOP dados abertos) — o que emenda financia por setor; cruza com acao_orcamentaria da indicação | siop | 2025 | há 1d | em dia |
| Saúde ASPS (SIOPS) | siops | 2025 | há 5h | em dia |
| Indicadores Previne + ISF (SISAB indicadorPainel — 10 quadrimestres) | sisab | 2025 | há 1d | em dia |
| Produção da APS (SISAB — fichas aprovadas, série mensal 2021+) | sisab | 2025 | há 6d | em dia |
| SNIS Água/Esgoto por município e prestador (atendimento, perdas, tratamento) — app Ministério das Cidades via Playwright; state-agnostic (UF/ANO) | snis | 2025 | há 1d | em dia |
| CAPAG capacidade de pagamento por município (STN/Tesouro) | tesouro | — | há 2h | em dia |
| Regularidade fiscal CAUC/CADIN (Tesouro) | tesouro | — | há 1d | em dia |
| Ranking da Qualidade da Informação Fiscal (Tesouro) por município | tesouro | 2025 | há 1d | em dia |
...
| Convênios e contratos de repasse por município (SICONV/Transferegov, repositório detru — proposta+convenio) | transferegov | 2025 | há 12d | em dia |
...
| Elegibilidade dos programas (Transferegov: quem pode captar cada programa — base do casamento oportunidade×necessidade) | transferegov | 2025 | há 1d | em dia |
...
- status: **ok** · registros suspeitos (excluídos): 6 · sobrepreço unitário: 18868
