...
> Gerada em 2026-09-02 07:14 -03 por `scripts/gerar_documentacao.mjs`. Reflete o estado real do código e do banco. **Não editar à mão.**
...
| `arquivo_texto_sc` | 672.647 | 17 (cnpj, ano, seq, sequencial_documento, cod_ibge, tipo_documento, titulo, texto…) |
| `arquivos_proc_feitos` | 255.294 | 3 (numero_controle, n, feito_em) |
| `arquivos_sc` | 672.762 | 14 (cnpj, ano, seq, sequencial_documento, cod_ibge, tipo_documento_id, tipo_documento, titulo…) |
...
| `atas_sc` | 123.578 | 12 (numero_controle_ata, cod_ibge, cnpj_orgao, ano_ata, numero_ata, numero_controle_compra, vigencia_inicio, vigencia_fim…) |
...
| `cadprev_sync_log` | 195 | 5 (id, recurso, uf, linhas, ts) |
...
| `catalogo_govbr_sc` | 23.021 | 7 (nivel, tipo, cod, nome, classe, grupo, atualizado_em) |
...
| `coleta_incremental_log` | 36 | 9 (rodada_em, uf, janela_ini, janela_fim, vistos, mudaram, iguais, novos…) |
...
| `contratacoes_sc` | 255.294 | 35 (cod_ibge, cnpj, ano, seq, esfera, plataforma, modalidade_id, modalidade…) |
| `contrato_sc` | 12.874 | 30 (cnpj, ano, seq, cod_ibge, uf, municipio_nome, numero_controle_pncp, numero_controle_compra…) |
| `contratos_sc` | 5.730.310 | 14 (id, cod_ibge, numero_controle_compra, cnpj_compra, ano_compra, seq_compra, fornecedor, ni_fornecedor…) |
...
| `crp_alertas` | 727 | 11 (id, cod_ibge, nome, eh_estado, evento, categoria_de, categoria_para, dias…) |
...
| `frescor_log` | 63 | 6 (id, rodado_em, total, ok, resumo, problemas) |
...
| `item_apresentacao_desc_sc` | 48.373 | 7 (chave, unidade_basica, fator, conf, metodo, n_itens, atualizado) |
| `item_apresentacao_sc` | 3.069 | 9 (unidade, unidade_basica, fator, forma, dimensao, conf, metodo, n_itens…) |
| `item_catmat_map` | 71.505 | 10 (chave, codigo_pdm, nome_pdm, nome_classe, sim, n_itens, atualizado, aceito…) |
...
| `item_catser_map` | 16.812 | 10 (chave, codigo_servico, nome_servico, nome_classe, nome_secao, sim, n_itens, aceito…) |
...
| `item_marca_sc` | 13.473 | 12 (cod_ibge, cnpj, ano, seq, numero, descricao, produto_ata, modelo…) |
...
| `item_resultado_sc` | 1.728.419 | 23 (cod_ibge, cnpj, ano, seq, numero, sequencial_resultado, ni_fornecedor, nome_razao_social_fornecedor…) |
...
| `itens_proc_feitos` | 255.262 | 4 (numero_controle, n, feito_em, versao) |
| `itens_sc` | 2.327.655 | 49 (cod_ibge, cnpj, ano, seq, numero, descricao, unidade, quantidade…) |
...
| `marca_ata_feitas` | 100.323 | 6 (cnpj, ano, seq, n_marcas, feito_em, n_propostas) |
| `marca_tpl_feitas` | 149.367 | 5 (cnpj, ano, seq, n_marca, feito_em) |
...
| `mislabel_unidade_sc` | 2.317 | 13 (cod_ibge, nome_pdm, descricao, unidade, unit_homologado, unidade_basica, pub, med…) |
...
| `notificacao_log` | 9.741 | 11 (id, cod_ibge, alerta_id, chave_delta, destinatario_id, canal, severidade, enviado_em…) |
...
| `pg_stat_statements` | 2.564 | 52 (userid, dbid, toplevel, queryid, query, plans, total_plan_time, min_plan_time…) |
...
| `pncp_evento` | 200.845 | 22 (cnpj, ano, seq, cod_ibge, categoria, acao, item_numero, resultado_sequencial…) |
...
| `precos_referencia_sc` | 1.918 | 13 (chave, unidade, mediana, p25, p75, n_itens, n_munis, catmat_pdm…) |
...
| `processos_sc` | 255.294 | 11 (cnpj_orgao, ano, sequencial, cod_ibge, numero_controle, modalidade_id, modalidade, objeto…) |
...
| `sobrepreco_compras_sc` | 5.549 | 15 (cod_ibge, chave, unidade, descricao, ano, quantidade, unit_pago, unit_ref…) |
...
|---|---|---|---|---|
| Outorgas de uso da água por município (ANA, finalidade+série) | ana | — | há 24d | em dia |
...
| Geração distribuída de energia por município (ANEEL, série) | aneel | — | há 21d | em dia |
| CFEM — royalties de mineração por município (ANM, distribuição) | anm | 2026 | há 24d | em dia |
| Preços de combustíveis por município (ANP, semestral, série 2004+) | anp | 2021 | há 24d | em dia |
| Vendas de combustíveis por município (ANP, série 1990+) | anp | 2024 | há 5h | em dia |
| Cobertura de planos de saúde por município (ANS — pressão sobre o SUS) | ans | 2026 | há 24d | em dia |
| CMED/Anvisa PMVG — preço-teto legal de medicamentos (Conformidade Gov, auto-descobre URL); referência nacional p/ sobrepreço em saúde (SC = ICMS 17%) | anvisa | 2025 | há 29d | em dia |
| Desastres S2ID por município (Atlas Digital CEPED-UFSC/MIDR, série 1991+) | atlas | — | há 5h | em dia |
| IDHM + subíndices por município (Atlas Brasil PNUD, Censo 2010) | atlasbrasil | 2025 | há 24d | em dia |
| Sistema financeiro por município (BCB Olinda — agências/cooperativas/correspondentes + Pix série) | bcb | — | há 1d | em dia |
...
| BNDES desembolsos por município (crédito produtivo, série 1995+) | bndes | 2026 | há 24d | em dia |
| Espelho completo CADPREV (37 recursos: DAIR/DIPR/DRAA/RPPS_*) | cadprev | — | há 36d | pendente |
| Alertas de CRP (transições vencido/a vencer — varredura) | cadprev | — | há 2h | em dia |
| Déficit atuarial RPPS (CADPREV/DRAA) | cadprev | — | há 17d | em dia |
...
| Habitação — MCMV (Minha Casa Minha Vida), unidades financiadas por município (Min. Cidades, gov.br/cidades — sem WAF); alimenta o casamento oportunidade×necessidade | cidades | 2025 | há 24d | em dia |
| Saneamento SINISA por município (água/esgoto/resíduos, Min. Cidades, ref 2024) | cidades | 2024 | há 24d | em dia |
| CNES — rede de saúde (Min. Saúde) | cnes | 2026 | há 5h | em dia |
| Unidades de conservação por município (MMA CNUC, interseção PostGIS) | cnuc | — | há 24d | em dia |
| Catálogo oficial CATMAT/CATSER (Compras.gov.br) — espinha de classificação | compras | — | há 5h | em dia |
| Referência NACIONAL de preços por CATMAT (Painel de Preços/Compras.gov.br), por unidade e por forma (avulso×escala) — enriquece o Banco de Preços e o sobrepreço | compras | 2025 | há 1d | pendente |
| PAA compras agricultura familiar por município (Conab) | conab | 2025 | há 5h | em dia |
| Bancada federal do estado (deputados + senadores) p/ módulo de Captação de Emendas — APIs Câmara + Senado | congresso | 2025 | há 29d | em dia |
...
| Lei Rouanet SALIC — aprovado/captado por município (MinC) | cultura | 2025 | há 26d | em dia |
| APAC oncologia e diálise por município (SIA-APAC) | datasus | — | há 5h | em dia |
| Equipamentos médicos por estabelecimento (CNES EQ) | datasus | 2024 | há 24d | em dia |
| Equipes de saúde (ESF) por município/estabelecimento (CNES EP) | datasus | 2024 | há 24d | em dia |
| Estabelecimentos de saúde por município (CNES — rede p/ regulação, API DEMAS) | datasus | 2025 | há 5h | em dia |
| Leitos hospitalares por estabelecimento (CNES LT) | datasus | 2024 | há 24d | em dia |
...
| Previne Brasil — indicadores APS (SISAB) | datasus | — | há 29d | em dia |
...
| Produção ambulatorial SUS por complexidade (DATASUS SIA + SIGTAP) | datasus | — | há 24d | em dia |
| Internações SUS por município (DATASUS SIH DBC mensal) | datasus | 2024 | há 24d | em dia |
| SIH — produção hospitalar (DATASUS TabNet) | datasus | 2025 | há 5h | em dia |
| Óbitos por município (DATASUS SIM, descompressor DBC próprio) | datasus | 2026 | há 24d | em dia |
| Agravos de notificação SINAN por município (tuberculose/hanseníase/violência) | datasus | 2024 | há 24d | em dia |
| Nascimentos por município (DATASUS SINASC DBC) | datasus | 2026 | há 24d | em dia |
| Apresentação Camada 2 (descrição → quantidade do conteúdo p/ itens-container); derivado de itens_sc + Camada 1, via SQL | derivado | 2025 | há 22d | em dia |
...
| Análise de compras por PREÇO UNITÁRIO — livro de preços de referência de SC + sobrepreço por município (derivado de itens_sc, via SQL) | derivado | 2025 | há 29d | em dia |
| Red flags de fornecedores — concentração + sanção (CEIS/CNEP) + sobrepreço por fornecedor (derivado de contratos/itens/sancoes, via SQL) | derivado | 2025 | há 29d | em dia |
| Re-derivação por fatia (evento → derivadas do ente) | derivado | 2025 | há 2h | pendente |
| Indícios de sobrepreço em medicamentos (compras do município vs teto legal PMVG, por substância+dosagem); derivado de cmed_pmvg + itens_sc | derivado | 2025 | há 24d | em dia |
| Reconstrução do sobrepreço com DUPLO benchmark — mediana de SC + referência nacional + desvio/CV (IN 65); derivado de itens_sc + precos_referencia_sc + precos_nacional_ref | derivado | 2025 | há 28d | em dia |
| Variação INTERNA de preços — mesmo município comprou o mesmo item a preços diferentes (derivado de itens_sc, via SQL) | derivado | 2025 | há 29d | em dia |
| Bolsa Atleta por município (Min. Esporte — atletas + valor) | esporte | 2025 | há 2h | pendente |
| Matrículas FUNDEB oficiais (FNDE Antonieta de Barros) | fnde | 2026 | há 24d | em dia |
| Parâmetros FUNDEB — fatores/VAAT/VAAR (FNDE) | fnde | 2026 | há 24d | em dia |
| PDDE por município (FNDE — Plataforma Antonieta de Barros) | fnde | 2024 | há 24d | em dia |
...
| PNLD reserva técnica — demanda de livros (FNDE — Antonieta de Barros) | fnde | 2025 | há 29d | em dia |
| Repasses federais FNS por bloco (Consulta Consolidada) | fns | 2026 | há 24d | em dia |
| Autos de infração ambiental por município (IBAMA, série 1990+) | ibama | — | há 29d | em dia |
...
| População por cor/raça Censo 2022 (IBGE SIDRA 9605) | ibge | 2025 | há 24d | em dia |
...
| Indicadores (IBGE/CGU) | ibge | 2024 | há 25d | em dia |
| População indígena (IBGE Censo 2022) | ibge | — | há 5h | em dia |
| IBGE MUNIC — instrumentos de gestão (planos/conselhos/fundos/instrumentos legais) por município, da BASE DE DADOS oficial (xlsx), não SIDRA | ibge | 2025 | há 23d | em dia |
...
| Saneamento por município (água/esgoto/lixo) — IBGE Censo 2022 via SIDRA (cobertura por domicílio) | ibge | 2025 | há 24d | em dia |
| Setores censitários Censo 2022 — perfil intraurbano (IBGE FTP) | ibge | 2025 | há 5h | em dia |
| % crianças 0-14 por setor → variável do mapa intraurbano (IBGE demografia) | ibge | 2025 | há 24d | em dia |
| Malha (polígonos) dos setores → mapa choropleth (IBGE GPKG) | ibge | 2025 | há 23d | em dia |
| % idosos por setor censitário → variável do mapa intraurbano (IBGE demografia) | ibge | 2025 | há 24d | em dia |
| Museus por município (IBRAM MuseusBr) | ibram | 2025 | há 24d | em dia |
| Assentamentos reforma agrária por município (INCRA/MDA SIPRA) | incra | — | há 5h | em dia |
| Censo Escolar — matrículas (INEP Sinopse) | inep | 2025 | há 23d | em dia |
...
| Escolas por município + infraestrutura (INEP Censo Escolar) | inep | 2025 | há 23d | em dia |
| Taxa de evasão por etapa (INEP Fluxo/Transição) | inep | 2025 | há 56d | em dia |
| FNDE liberações por município (SIMAD, Playwright) | inep | 2025 | há 5h | em dia |
| IDEB — indicadores educacionais (INEP) | inep | 2025 | há 24d | em dia |
| Indicadores educacionais INEP (AFD/TDI/ATU/rendimento por município) | inep | 2025 | há 23d | em dia |
| Indicadores INEP por escola (georreferenciado) | inep | 2025 | há 23d | em dia |
| SAEB proficiência Port/Mat por etapa (INEP) | inep | 2025 | há 5h | em dia |
| Arboviroses (dengue/zika/chikungunya) por município (InfoDengue/SINAN) | infodengue | 2025 | há 1h | pendente |
| Dengue por município (SINAN via InfoDengue, série + incidência) | infodengue | 2025 | há 5h | em dia |
| Desmatamento PRODES por município (INPE, interseção PostGIS) | inpe | 2025 | há 24d | em dia |
| Focos de calor por município (INPE BDQueimadas, mensal) | inpe | 2026 | há 11d | em dia |
| IEGM — qualidade da gestão (TCE-SC/IRB, dados abertos) | irb | 2025 | há 24d | em dia |
...
| CAF — Cadastro Nacional da Agricultura Familiar (ex-DAP) por município: nº de agricultores familiares (MDA, XLSX mensal) | mda | 2025 | há 29d | em dia |
...
| Saldo de empregos formais por município (Novo CAGED/MTE, mensal) | mte | 2026 | há 29d | em dia |
| RAIS — estoque de emprego formal por município (MTE, anual) | mte | 2025 | há 23d | em dia |
| Novo PAC obras por município (ObrasGov/Casa Civil) | obrasgov | 2025 | há 19d | em dia |
| Obras por município — detalhe (ObrasGov) | obrasgov | 2025 | há 24d | em dia |
| Equipamentos esportivos georreferenciados (OSM — mapa camada Esporte) | osm | — | há 24d | em dia |
| Equipamentos segurança/justiça/defesa civil georreferenciados (OSM + SAP/SC): polícia, bombeiros, defesa civil, prisional, socioeducativo | osm | — | há 24d | em dia |
| Atas de Registro de Preço (PNCP Consulta) | pncp | — | há 29d | pendente |
| Classificação dos itens → CATMAT/CATSER (dicionário, matcher v2) | pncp | — | há 1d | pendente |
| Busca diária do PNCP (compras/contratos/atas — ano corrente) | pncp | — | há 11d | em dia |
| Contratos (PNCP ano corrente, append) | pncp | 2027 | há 47d | pendente |
| Empenhos por contrato (PNCP Lei 14.133 — acende quando publicarem) | pncp | — | há 3h | em dia |
...
| Itens de TODOS os processos (preço unitário) | pncp | 2025 | há 1d | em dia |
| Notas fiscais / instrumentos de cobrança (PNCP — acende quando publicarem) | pncp | — | há 24d | pendente |
| PCA (PNCP) | pncp | — | há 4h | em dia |
| Preço de referência por item (mediana SC) + classificação ata/efetivada — base da análise de preços | pncp | 2025 | há 71d | em dia |
| Contratações PNCP — TODAS (entidade Contratação espelhando o PNCP: modalidade/plataforma/SRP/estimado×homologado; via bulk por data) | pncp | 2025 | há 1d | em dia |
| Sazonalidade de preço por categoria (melhor mês de compra, SC) | pncp | 2025 | há 70d | em dia |
| Acidentes em rodovias federais por município (PRF DATATRAN, série 2015+) | prf | 2025 | há 29d | em dia |
| Localidade dos fornecedores (CNPJ→UF/município) | receita | — | há 9d | em dia |
| Arrecadação federal por município (RFB, série 2019+) | rfb | 2025 | há 24d | em dia |
| Frota de veículos por município (SENATRAN) | senatran | 2025 | há 24d | em dia |
| Qualidade (15 indicadores) + Vínculo/CVAT — novo cofinanciamento (SIAPS) | siaps | 2025 | há 5h | em dia |
| CAR — Cadastro Ambiental Rural: nº de imóveis rurais por município (SICAR GeoServer WFS, contagem por município) | sicar | 2025 | há 24d | em dia |
| Acompanhamento intra-anual da execução (RREO do bimestre vigente) — receita prevista×realizada e despesa orçada×empenhada por município | siconfi | 2025 | há 25d | em dia |
| Acompanhamento por função (intra-anual) — orçado×realizado por função até o bimestre vigente (RREO Anexo 02 parcial) | siconfi | 2025 | há 25d | em dia |
| CNPJs do governo municipal (SICONFI+órgãos+RPPS) | siconfi | 2025 | há 25d | em dia |
| Despesa por subfunção (RREO an.2 — drill) | siconfi | 2025 | há 5h | em dia |
| Finanças (SICONFI RREO an.1/2) | siconfi | 2025 | há 5d | em dia |
| Metas Fiscais LDO (RREO an.6) | siconfi | 2025 | há 5h | em dia |
| MSC ancorada ao RREO — despesa empenhada por natureza e fonte (forma da MSC × total exato do RREO; reconcilia por construção) | siconfi | 2025 | há 25d | em dia |
...
| Salário-Educação por município (SICONFI DCA) | siconfi | 2025 | há 24d | em dia |
| Estatísticas vitais por município (IBGE Registro Civil — nascidos/óbitos, série) | sidra | 2024 | há 28d | em dia |
| Produção agropecuária (PAM/PPM) + empresas (CEMPRE) por município (IBGE SIDRA) | sidra | — | há 24d | em dia |
| Vítimas de crimes violentos letais por município (SINESP/MJSP, dados abertos) | sinesp | — | há 59d | em dia |
| Catálogo de Ações Orçamentárias Federais (SIOP dados abertos) — o que emenda financia por setor; cruza com acao_orcamentaria da indicação | siop | 2025 | há 24d | em dia |
| Saúde ASPS (SIOPS) | siops | 2025 | há 5h | em dia |
| Indicadores Previne + ISF (SISAB indicadorPainel — 10 quadrimestres) | sisab | 2025 | há 24d | em dia |
| Produção da APS (SISAB — fichas aprovadas, série mensal 2021+) | sisab | 2025 | há 29d | pendente |
| SNIS Água/Esgoto por município e prestador (atendimento, perdas, tratamento) — app Ministério das Cidades via Playwright; state-agnostic (UF/ANO) | snis | 2025 | há 24d | em dia |
...
| Ranking da Qualidade da Informação Fiscal (Tesouro) por município | tesouro | 2025 | há 24d | em dia |
| Transferências da União por município, MENSAL (FPM/FUNDEB/ITR/Lei Kandir/CIDE/FEX/IOF/LC176) — CSV oficial STN/Tesouro Transparente; NACIONAL, state-agnostic (UF env) | tesouro | 2025 | há 24d | em dia |
...
| Assistência social COMPLETA (MDS MI Social): série anual de repasse FNAS 2005→ + CadÚnico + Bolsa Família | transferegov | 2025 | há 24d | em dia |
| Convênios e contratos de repasse por município (SICONV/Transferegov, repositório detru — proposta+convenio) | transferegov | 2025 | há 35d | pendente |
| Emendas — INDICAÇÃO (SICONV/Transferegov, repositório detru: parlamentar, impositivo, valor destinado) | transferegov | 2025 | há 24d | em dia |
...
| Sanções a empresas/pessoas (CEIS + CNEP) — API Portal da Transparência; cruza com fornecedores (fornecedor sancionado) | transparencia | 2025 | há 14d | em dia |
...
- status: **ok** · registros suspeitos (excluídos): 6 · sobrepreço unitário: 19208
