...
console.log(`[ao_homologar UF=${UF}] watermark = ${wm || "(início)"}`);

// 1) EVENTOS — processos com item (des)homologado/retificado desde o watermark
const eventos = (await q(`
  SELECT DISTINCT cnpj,ano,seq FROM ${ITENS}
  WHERE ($1::timestamptz IS NULL OR data_atualizacao > $1)`, [wm])).rows;
console.log(`eventos de homologação/retificação: ${eventos.length} processos`);

if (eventos.length) {
  // 2) REABRE (invalida idempotência) → extrator reconcilia
  const rReabre = await q(`DELETE FROM ${FEITAS} f USING ${ITENS} i
    WHERE f.cnpj=i.cnpj AND f.ano=i.ano AND f.seq=i.seq AND ($1::timestamptz IS NULL OR i.data_atualizacao > $1)`, [wm]);
  // 4) RE-ENRIQUECER: remove do enriquecimento os processos reabertos (voltam pra fila do enriquecedor)
  const rEnr = await q(`DELETE FROM app.item_enriquecimento e USING ${ITENS} i
    WHERE e.cnpj=i.cnpj AND e.ano=i.ano AND e.seq=i.seq AND ($1::timestamptz IS NULL OR i.data_atualizacao > $1)`, [wm]);
  // 3) FETCH: processos com item HOMOLOGADO mas SEM doc de resultado no acervo → enfileira p/ baixar do portal
  const rFetch = await q(`
    ...
    FROM ${ITENS} i
    WHERE ($1::timestamptz IS NULL OR i.data_atualizacao > $1) AND i.unit_homologado IS NOT NULL
      AND NOT EXISTS (SELECT 1 FROM ${ARQ} a WHERE a.cnpj=i.cnpj AND a.ano=i.ano AND a.seq=i.seq AND a.titulo ~* '${RES_REGEX}')
    ON CONFLICT(cnpj,ano,seq) DO NOTHING`, [wm]);
...
if (novoWm) await q(`INSERT INTO app.auditoria_watermark(chave,ts) VALUES($1,$2)
  ON CONFLICT(chave) DO UPDATE SET ts=EXCLUDED.ts`, [WM_KEY, novoWm]);
console.log(`watermark avançado p/ ${novoWm}. Extrator/enriquecedor (tasks) consomem a fila reaberta; fetch em ${FETCH}.`);
await db.end();
