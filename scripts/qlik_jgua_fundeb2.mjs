...
// 1) descobrir os cargos de merendeira
const cargos=(await cube(["nomeCargo"],[BRUTO],600)).map(r=>r[0].qText||"").filter(c=>/aliment|merend/i.test(c));
console.log("cargos merendeira:", cargos.length, "→", JSON.stringify(cargos));
// 2) selecionar esses cargos e agrupar por lotação (=fonte)
await selMany("nomeCargo",cargos);
const byLot=(await cube(["descricaoLotacao"],["Count(DISTINCT numeroCPF)",BRUTO],400)).map(r=>({lot:r[0].qText||"",n:+r[1].qNum||0,v:+r[2].qNum||0})).filter(r=>r.n>0);
console.log("\n=== MERENDEIRAS por LOTAÇÃO (=fonte) 202511 ===");
let tF=0,tO=0; byLot.sort((a,b)=>b.v-a.v).forEach(r=>{const fu=/fundeb/i.test(r.lot);if(fu)tF+=r.v;else tO+=r.v;console.log(`  ${fu?'[FUNDEB]':'[outra ]'} ${r.lot.slice(0,46).padEnd(46)} ${String(r.n).padStart(4)} | R$ ${Math.round(r.v).toLocaleString('pt-BR')}`);});
console.log(`\n>>> FUNDEB R$ ${Math.round(tF).toLocaleString('pt-BR')} (${Math.round(tF/(tF+tO)*100)}%) · outras fontes R$ ${Math.round(tO).toLocaleString('pt-BR')} (${Math.round(tO/(tF+tO)*100)}%)`);
ws.close();
