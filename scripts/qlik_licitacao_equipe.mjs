...
const LOTS=["GERENCIA DO SISTEMA DE COMPRAS","DIRETORIA DE LICITACOES","DIR DO SISTEMA DE LICITACOES E CONTRATOS","SUBSECR DE LICIT CONTRATOS E CONCESSOES","SUPERINTENDEN DE LICITACOES E CONTRATOS","ASSESSORIA DE COMPRAS","DIRETORIA DE CONTRATOS","GERENCIA DO SISTEMA DE CONTRATOS"];
const AM="202506";
const todos=[];
for(const L of LOTS){
  await rpc("ClearAll",appH,[false]);
  await selVal("nomeUG","Prefeitura Municipal de Florianópolis"); await selVal("anoMes",AM); await selVal("descricaoLotacao",L);
  const r=await cube(["nome","nomeCargo"],[BRUTO],40);
  console.log(`\n[${L}]`);
  r.map(x=>({nome:x[0].qText,cargo:x[1].qText,bruto:x[2].qNum,lotacao:L})).filter(x=>x.nome&&x.nome!=="-").sort((a,b)=>b.bruto-a.bruto)
   .forEach(x=>{ todos.push(x); console.log(`  ${x.nome} | ${x.cargo} | bruto R$ ${x.bruto.toLocaleString("pt-BR",{minimumFractionDigits:2})}`); });
}
const tb=todos.reduce((s,x)=>s+x.bruto,0);
console.log(`\n=== TOTAL equipe de licitações/compras/contratos: ${todos.length} servidores · R$ ${tb.toLocaleString("pt-BR",{minimumFractionDigits:2})}/mês (jun/2025) ===`);
fs.writeFileSync(OUT+"folha_licitacao_equipe.json", JSON.stringify({anoMes:AM,servidores:todos},null,1));
ws.close();
