...
async function main() {
  ...
  await db.query(`CREATE TABLE IF NOT EXISTS fnde_simad_check (cod_ibge TEXT, ano INTEGER, n INTEGER, PRIMARY KEY (cod_ibge, ano))`);
  ...
  const feitos = new Set((await db.query(`SELECT cod_ibge||'-'||ano k FROM fnde_simad_check`)).rows.map((r) => r.k));
  ...
  for (const e of entes) {
    for (let ano = ANO_FIM; ano >= ANO_INI; ano--) {
      ...
      try {
        ...
        // tp=02 (PREFEITURA) → vai DIRETO à tabela do governo municipal (sem listar as ~160 escolas/UEx do PDDE).
        await page.evaluate(({ ano, mun }) => { const s = (n, v) => { const el = document.querySelector(`[name="${n}"]`); if (el) el.value = v; }; s("p_ano", String(ano)); s("p_municipio", mun); s("p_tp_entidade", "02"); s("p_programa", ""); s("p_cgc", ""); s("p_verifica", "sigef"); }, { ano, mun: e.ibge6 });
        ...
        if (cnpjs.length) {
          ...
          linhas.push(...await page.evaluate(PARSE)); // resultado direto (1 entidade)
        }
      ...
      await q(`INSERT INTO fnde_simad_check (cod_ibge,ano,n) VALUES ($1,$2,$3) ON CONFLICT (cod_ibge,ano) DO UPDATE SET n=EXCLUDED.n`, [e.ibge7, ano, linhas.length]);
      await sleep(200);
    }
    proc++;
