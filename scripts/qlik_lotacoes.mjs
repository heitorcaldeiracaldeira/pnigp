...
// TODAS as lotações + nº servidores (analisar, não chutar)
const lots=await cube(["descricaoLotacao"],["Count(DISTINCT numeroCPF)"],2000);
console.log("total lotações na Prefeitura:",lots.length);
const alvo=lots.filter(r=>/ALIMENT|MERENDA|COORDENADORIA|OPERACIONAL|DAOP|NUTRI/i.test(r[0].qText));
console.log("\n=== lotações candidatas (aliment/coordenadoria/operacional/nutri) ===");
alvo.forEach(r=>console.log(`  [${r[1].qText} serv] ${r[0].qText}`));
fs.writeFileSync(OUT+"farol_lotacoes_floripa.json", JSON.stringify(lots.map(r=>({lotacao:r[0].qText,serv:r[1].qNum})),null,1));
ws.close();
