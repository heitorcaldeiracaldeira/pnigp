...
const FONTES = [
  ...
  // NÃO era duplicata, era COLISÃO DE ID: duas coletas diferentes chamadas "sih". Esta traz internação pelo
  // DBC mensal e escreve em sih_sc; a outra (sih_producao) traz produção hospitalar do TabNet e escreve em
  // saude_producao_sc. Como o id era o mesmo, as duas dividiam uma linha de estado no catálogo e o mapa TAB
  // ficava com a última — logo esta aqui era vigiada contra a tabela da outra. Ids distintos, cada uma com o
  // seu estado e a sua tabela. Renomear é seguro: nada fora do orquestrador referencia estes ids.
  { id: "sih_internacoes", label: "Internações SUS por município (DATASUS SIH DBC mensal)", api: "datasus", script: "scripts/ingest_datasus_sih_sc.mjs", env: {}, devido: async (st) => diasDesde(st?.ultima_exec) > 60 },
  ...
  { id: "sih_producao", label: "SIH — produção hospitalar (DATASUS TabNet)", api: "datasus", script: "scripts/ingest_sih_sc.mjs", env: { ANOS: `${ANO_FECHADO},${ANO_CORRENTE}` },
    devido: async (st) => diasDesde(st?.ultima_exec) > 30 }, // competência mensal (TabNet); ver nota em sih_internacoes
  ...
  // ERA DUAS ENTRADAS (05/ago/2026): o mesmo script estava registrado como api "sicor" (cadência 3 dias) e como
  // api "bcb" (90 dias). Grupos de API diferentes rodam em PARALELO, então o mesmo ETL rodava duas vezes ao
  // mesmo tempo, na mesma pronaf_sc, disputando uma única linha de estado no catálogo. Ficou uma entrada só,
  // no grupo "bcb" — SICOR é sistema do próprio Banco Central, e serializar com as outras chamadas ao BCB
  // evita martelar o mesmo host. A cadência curta da entrada removida foi preservada: o servidor é instável e
  // falha aqui quase sempre é indisponibilidade, não ausência de dado.
  { id: "pronaf", label: "PRONAF / Crédito Rural por município — valor contratado por ano (BCB SICOR/Matriz do Crédito Rural, OData; cdEstado=25)", api: "bcb", script: "scripts/ingest_pronaf_sc.mjs", env: {},
    devido: async (st) => Number((await db.query(`SELECT count(*) n FROM pronaf_sc`).catch(() => ({ rows: [{ n: 0 }] }))).rows[0].n) === 0 || diasDesde(st?.ultima_exec) > 3 },
...
async function main() {
  ...
  if (MODO === "run") {
    ...
    // são VALIDAÇÕES: uma auditoria de integridade que reprovasse, ou um vazamento de dado do Estado dentro
    // do município, terminavam sem interromper, sem marcar e sem aparecer em lugar nenhum. Continuam não
    // interrompendo — são posteriores à coleta e não adianta desfazer o ciclo — mas agora o resultado é dito.
    const rodarScript = (s) => new Promise((res) => {
      const c = spawn(process.execPath, [s], { cwd: ROOT, stdio: "ignore" });
      c.on("exit", (code) => res(code ?? -1)); c.on("error", () => res(-1));
    });
    const passo = async (rotulo, script, ehValidacao = false) => {
      log(`${rotulo}…`);
      const code = await rodarScript(script);
      if (code !== 0) log(`   ${ehValidacao ? "!! REPROVOU" : "!! falhou"}: ${script} saiu ${code}`);
      return code;
    };
    const reprovas = [];
    if (await passo("validando integridade", "scripts/auditoria_dados_sc.mjs", true) !== 0) reprovas.push("integridade");
    if (await passo("validando separação Estado×município", "scripts/validacao_estado_vazamento.mjs", true) !== 0) reprovas.push("Estado×município");
    await passo("varredura de frescor (série histórica + última competência)", "scripts/varredura_frescor.mjs");
    await passo("motor de notificações (delta — avisa o que mudou)", "scripts/motor_notificacoes.mjs");
    await passo("carteiro de notificações (envia se houver SMTP; simula caso contrário)", "scripts/enviar_notificacoes.mjs");
    await passo("carteiro WhatsApp (envia se houver Cloud API; simula caso contrário)", "scripts/enviar_notificacoes_whatsapp.mjs");
    await passo("regenerando documentação", "scripts/gerar_documentacao.mjs");
    if (reprovas.length) log(`!! CICLO CONCLUÍDO COM VALIDAÇÃO REPROVADA: ${reprovas.join(", ")} — o dado foi coletado, mas confira antes de publicar`);
  }
  log("ciclo concluído.");
  await trava?.solta();
  await db.end();
