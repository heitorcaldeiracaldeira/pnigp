...
export function IndicadoresInep({ data, nome }: { data: NonNullable<IndicadoresInepSC>; nome: string }) {
  const d = data;
  const csvRows = [
    { indicador: "AFD — formação docente adequada (%)", municipio: d.afd.funAi, mediana_sc: d.medSC.afdAi },
    { indicador: "TDI — distorção idade-série (%)", municipio: d.tdi.funAi, mediana_sc: d.medSC.tdiAi },
    { indicador: "ATU — alunos por turma", municipio: d.atu.funAi, mediana_sc: d.medSC.atuAi },
    { indicador: "Taxa de aprovação (%)", municipio: d.aprovacao.funAi, mediana_sc: d.medSC.aprovAi },
    { indicador: "Taxa de abandono (%)", municipio: d.abandono.funAi, mediana_sc: d.medSC.abandAi },
  ];
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <BaixarCsv nome={`indicadores-inep-${nome}`} label="CSV" linhas={csvRows as unknown as Record<string, unknown>[]} colunas={[{ chave: "indicador", rotulo: "Indicador (anos iniciais)" }, { chave: "municipio", rotulo: "Município" }, { chave: "mediana_sc", rotulo: "Mediana SC" }]} />
