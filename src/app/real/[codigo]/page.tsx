...
export default async function RealEntePage({ params }: { params: Promise<{ codigo: string }> }) {
  ...
  const [repassesStn, pecaCompleta, ppaPrograma, mscDespesa, fornecSancionados, sobrepreco, variacaoInterna, redFlags, mislabel, acompanhamento, acompFuncao, programasFederais, catalogoProgramas, sazPreco, fornec, comprasExtra, tceApont, tceProcs] = await Promise.all([
    getTransferenciasStnSC(codigo), getPecaCompletaSC(codigo), getPpaProgramaSC(codigo), getMscDespesaSC(codigo), getFornecedoresSancionadosSC(codigo), getSobreprecoSC(codigo), getVariacaoInternaSC(codigo), getRedFlagsSC(codigo), getMislabelUnidadeSC(codigo), getAcompanhamentoSC(codigo), getAcompanhamentoFuncaoSC(codigo), getProgramasFederaisSC(), getCatalogoProgramasSC(), getSazonalidadePrecoSC(), getFornecedoresSC(codigo), getComprasExtraSC(codigo), getTceApontamentos(codigo), getTceProcessosApontados(codigo)]);
  ...
  const tabs: { id: string; label: string; content: React.ReactNode; grupo?: string }[] = [
    ...
    { id: "compras", label: "Compras", content: <>
      ...
      {tceApont && <div className="mt-4"><TceApontamentosCard dados={tceApont} municipio={ente.nome} processos={tceProcs} /></div>}
