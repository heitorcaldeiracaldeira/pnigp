...
export default async function RealEntePage({ params }: { params: Promise<{ codigo: string }> }) {
  ...
  if (saude) {
    ...
    tabs.push({ id: "saude", label: "Saúde — visão geral", content: (
      <>
        ...
        <div className="mt-4"><AnaliseSaude previne={previne} fns={fns} saude={saude} nome={ente.nome} /></div>{indAps && <div className="mt-4"><IndicadoresApsPanel d={indAps} /></div>}{qualInd && <div className="mt-4"><QualidadeIndicadoresApsPanel d={qualInd} /></div>}{dinMesa && <div className="mt-4"><DinheiroMesaApsPanel d={dinMesa} /></div>}{vinculo && <div className="mt-4"><VinculoApsPanel d={vinculo} /></div>}{prodAps && <div className="mt-4"><ProducaoApsPanel d={prodAps} /></div>}{cobAps && <div className="mt-4"><CoberturaApsPanel d={cobAps} /></div>}{finAps && <div className="mt-4"><FinanciamentoApsPanel d={finAps} /></div>}
  ...
  if (assistSocial) {
    ...
    tabs.push({ id: "assistencia", label: "Assistência — visão geral", content: <>
      ...
      {igdm && <div className="mt-4"><IgdmPanel d={igdm} /></div>}{suasSaldo && <div className="mt-4"><SuasSaldoPanel d={suasSaldo} /></div>}{pddeSaldo && <div className="mt-4"><PddeSaldoPanel d={pddeSaldo} /></div>}{pnaeAgri && <div className="mt-4"><PnaeAgriPanel d={pnaeAgri} /></div>}{paa && <div className="mt-4"><PaaPanel d={paa} /></div>}{barragens && <div className="mt-4"><BarragensPanel d={barragens} /></div>}{lpg && <div className="mt-4"><LpgPanel d={lpg} /></div>}{salic && <div className="mt-4"><SalicPanel d={salic} /></div>}{novopac && <div className="mt-4"><NovoPacPanel d={novopac} /></div>}{pib && <div className="mt-4"><PibMunicipalPanel d={pib} /></div>}{idhm && <div className="mt-4"><IdhmPanel d={idhm} /></div>}{cemaden && <div className="mt-4"><CemadenPanel d={cemaden} /></div>}{domicilios && <div className="mt-4"><DomiciliosPanel d={domicilios} /></div>}{alfab && <div className="mt-4"><AlfabetizacaoPanel d={alfab} /></div>}{setores && <div className="mt-4"><SetoresPanel d={setores} codigo={codigo} /></div>}{museus && <div className="mt-4"><MuseusPanel d={museus} /></div>}{popFaixa && <div className="mt-4"><PopulacaoFaixaPanel d={popFaixa} /></div>}{censoCR && <div className="mt-4"><CensoCorRacaPanel d={censoCR} /></div>}{quilombos && <div className="mt-4"><QuilombosPanel d={quilombos} /></div>}
  ...
  if (queimadas || prodes) tabs.push({ id: "meio_ambiente", label: "Meio Ambiente e Defesa Civil", content: (
    <div className="space-y-3">
      ...
      {prodes && <ProdesPanel d={prodes} />}{ibamaAutos && <IbamaAutosPanel d={ibamaAutos} />}{ibamaEmbargos && <IbamaEmbargosPanel d={ibamaEmbargos} />}{icmbioUc && <IcmbioUcPanel d={icmbioUc} />}{anaOutorgas && <AnaOutorgasPanel d={anaOutorgas} />}{mapaAmbiental && <MapaAmbiental data={mapaAmbiental} nome={ente.nome} />}
    </div>
  ) });
  ...
  if (escolas) tabs.push({ id: "equipamentos", label: "Equipamentos Públicos", content: <>
    <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-teal-50 to-white p-5">
      ...
      <p className="text-sm text-slate-500">As escolas da rede municipal de {ente.nome}, uma a uma, com infraestrutura e lacunas — o nível mais concreto da gestão. (Saúde e Assistência terão suas próprias páginas de equipamentos.)</p>
    </div>
