...
import { TceApontamentosCard } from "@/components/tce-apontamentos";
...
export default async function RealEntePage({ params }: { params: Promise<{ codigo: string }> }) {
  ...
  const [agropec, captacaoEmendas, cadernoProgramas, emendasEstaduais, emendasEstObjetos, lacunaEdu, lacunaSaude, lacunaAssist, extracao, incraAssent, pronaf, ibgeProducao, merenda] = await Promise.all([getAgropecuariaSC(codigo), getCaptacaoEmendasSC(codigo), getCadernoProgramasSC(codigo), getEmendasEstaduaisSC(codigo), getEmendasEstObjetosSC(), getLacunaCaptacaoEducacaoSC(codigo), getLacunaCaptacaoSaudeSC(codigo), getLacunaCaptacaoAssistenciaSC(codigo), getCatalogoExtracao(), getIncraAssentamentosSC(codigo), getPronafSC(codigo), getIbgeProducaoSC(codigo), getMerendaSC(codigo)]);
  const [repassesStn, pecaCompleta, ppaPrograma, mscDespesa, fornecSancionados, sobrepreco, variacaoInterna, redFlags, mislabel, acompanhamento, acompFuncao, programasFederais, catalogoProgramas, sazPreco, fornec, comprasExtra, tceApont, tceProcs, tceDiverg] = await Promise.all([
    getTransferenciasStnSC(codigo), getPecaCompletaSC(codigo), getPpaProgramaSC(codigo), getMscDespesaSC(codigo), getFornecedoresSancionadosSC(codigo), getSobreprecoSC(codigo), getVariacaoInternaSC(codigo), getRedFlagsSC(codigo), getMislabelUnidadeSC(codigo), getAcompanhamentoSC(codigo), getAcompanhamentoFuncaoSC(codigo), getProgramasFederaisSC(), getCatalogoProgramasSC(), getSazonalidadePrecoSC(), getFornecedoresSC(codigo), getComprasExtraSC(codigo), getTceApontamentos(codigo), getTceProcessosApontados(codigo), getTceDivergenciasValor(codigo)]);
  // Cauda paralela (antes: 4 awaits em série = 4 round-trips sequenciais após o último batch).
  const [estabSaude, perfilSaude, perfilEdu, censoTend] = await Promise.all([
    getEstabSaudeSC(codigo), getPerfilSaudeSC(codigo), getPerfilEducacaoSC(codigo), getCensoTendenciaSC(codigo)]);
  ...
  const medianaInvestSC = investOrd.length ? (investOrd.length % 2 ? investOrd[Math.floor(investOrd.length / 2)] : (investOrd[investOrd.length / 2 - 1] + investOrd[investOrd.length / 2]) / 2) : 0;
  ...
  const tabs: { id: string; label: string; content: React.ReactNode; grupo?: string }[] = [
    ...
    { id: "compras", label: "Compras", content: <>
      ...
      {tceApont && <div className="mt-4"><TceApontamentosCard dados={tceApont} municipio={ente.nome} processos={tceProcs} divergencias={tceDiverg} /></div>}
    ...
    { id: "processo-fases", label: "Processo Licitatório", content: <>
      ...
      <OrcamentoObraPainel nome={ente.nome} />
  ...
  if (merenda) tabs.push({ id: "custo-merenda", label: "Custo da Merenda Escolar", content: <>
    <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
      <Carimbo fonte="Censo INEP · FNDE/SIMAD (PNAE) · PNCP · e-Pública (aprofundamento)" competencia="custo da alimentação escolar ao cidadão" extraido={extracao.financas} />
      {merenda.pnaeSerie.length > 0 && <BaixarCsv nome={`pnae-merenda-${ente.nome}`} label="Baixar PNAE por ano (CSV)" colunas={[{ chave: "ano", rotulo: "Ano" }, { chave: "valor", rotulo: "PNAE recebido" }]} linhas={merenda.pnaeSerie as unknown as Record<string, unknown>[]} />}
    </div>
    <AssuntoMerenda dados={merenda} curado={merendaCurado(codigo)} nome={ente.nome} />
  </> });
  if (assistSocial) {
  ...
  const GRUPOS: [string, string[]][] = [
    ...
    ["Base Econômica", ["financeiro", "rais", "emprego", "agropecuaria", "economia", "populacao"]],
    ["Segurança", ["seguranca"]],
  ...
  // Guarda de dev: aba sem grupo cai em "Geral" (foi o bug que originou a revisão). Avisa no build/dev.
  if (process.env.NODE_ENV !== "production") {
    const orfas = tabs.filter((t) => !t.grupo).map((t) => t.id);
    if (orfas.length) console.warn(`[page /real] abas órfãs sem grupo (caem em "Geral"): ${orfas.join(", ")} — adicione ao mapa GRUPOS`);
  }
