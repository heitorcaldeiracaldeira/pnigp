...
import { Building2, FileDown, LayoutDashboard, MapPin, Star, Wallet } from "lucide-react";
...
export default async function GovernadorPage({
  ...
  const [estados, serie, indicadores, ranking, metas, historico, financas, compras, contratacoes] =
    await Promise.all([
      getEstados(),
  ...
  const orcExecData = fin
    ? despesaFuncaoArvore(fin, estado.uf).map((n) => ({ label: n.nome, orcado: n.previsto, executado: n.realizado }))
    : [];
  const recOrigData = fin
    ...
    : [];
  const forn = resumoFornecedores(contratacoes, estado.uf);
  const fornData = [
    { label: "Do estado", valor: forn.local.valor },
  ...
  const info = [
    { icon: MapPin, label: "Região", valor: estado.regiao },
    { icon: Building2, label: "Capital", valor: estado.capital },
    { icon: Star, label: "População", valor: fmtPop(estado.populacao) },
    { icon: Wallet, label: "PIB per capita", valor: fmtBRL(estado.pib_per_capita) },
  ];
        ...
        <>
          ...
          <section aria-label="Panorama para decisão">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <PanoramaCard href="#auditoria" titulo="Desempenho por área" sub="Comparado a pares da região (50 = média)">
              ...
              {contratacoes.length > 0 && (
                <PanoramaCard href="#compras" titulo="Fornecedores" sub="Valor contratado: do estado × de fora">
                  <Donut data={fornData} />
                </PanoramaCard>
              )}
              {contratacoes.length > 0 && (
    ...
    },
    {
      ...
      content: (
        <section aria-label="Indicadores setoriais" className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <IndicatorsSection indicadores={indicadores} benchmarkLabel="Média da região" />
        </section>
      ),
    },
    {
      ...
      content: financas.atual ? (
        <FinancasPanel financas={financas.atual} anterior={financas.anterior} populacao={estado.populacao} seed={estado.uf} tipo="E" />
      ) : (
    ...
    },
    {
      ...
      content: (
        <section aria-label="Benchmarking nacional" className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <p className="mb-4 text-xs text-slate-500">Ranking das unidades federativas por IGP 360 · 2024</p>
          <RankingTable ranking={rankRows} codigoAtual={estado.uf} grupoHeader="Região" destaqueLabel="SEU ESTADO" />
        </section>
      ),
    },
  ];
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500">
            Governador(a): <strong className="text-slate-700">{estado.governador}</strong> · Dados do exercício 2024 (último consolidado)
          </p>
        </div>
        <div className="flex flex-col items-start gap-3 lg:items-end">
          <div className="flex flex-col items-start gap-1 lg:items-end">
            <span className="text-xs text-slate-500">Trocar estado</span>
            <EstadoSelector estados={estados} atual={estado.uf} />
          </div>
          <div className="flex gap-2">
            <Link
              href={`/governador/${estado.uf}/gestao`}
            ...
            <Link
              href={`/relatorio/estado/${estado.uf}`}
